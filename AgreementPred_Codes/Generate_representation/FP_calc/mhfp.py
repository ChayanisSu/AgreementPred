...
class MHFPEncoder:
  ...
  """
    ...
    """
        ...
        self.permutations_a = np.zeros([n_permutations], dtype=np.uint64)
        self.permutations_b = np.zeros([n_permutations], dtype=np.uint64)
        ...
        for i in range(n_permutations):
            a = rand.randint(1, MHFPEncoder.max_hash, dtype=np.uint64)
            b = rand.randint(0, MHFPEncoder.max_hash, dtype=np.uint64)
    ...
    """
        ...
        hash_values = np.zeros([self.n_permutations, 1], dtype=np.uint64)
